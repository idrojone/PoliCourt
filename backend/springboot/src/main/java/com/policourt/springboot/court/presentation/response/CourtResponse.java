...
public record CourtResponse(
    ...
    String location,
    @Schema(description = "URL de la imagen de la pista", example = "http://example.com/image.jpg")
    String imgUrl,
    ...
    @Schema(description = "Lista de deportes disponibles en la pista", example = "[\"FUTBOL\", \"BALONCESTO\"]")
    List<String> sportsAvailable
) {
    public static CourtResponse fromDomain(Court court) {
        var sportsAvailable = court.getSportCourts() == null ? List.<String>of() :
            court.getSportCourts().stream()
                .map(cs -> cs.getSport() == null ? null : cs.getSport().getName())
