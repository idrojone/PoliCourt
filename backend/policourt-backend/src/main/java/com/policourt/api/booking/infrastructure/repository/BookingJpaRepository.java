...
public interface BookingJpaRepository extends JpaRepository<BookingEntity, Long>, JpaSpecificationExecutor<BookingEntity> {
    ...
    Optional<BookingEntity> findByTitle(String title);
        ...
        @Query(value = "SELECT 1 FROM bookings WHERE court_id = :courtId AND start_time = :startTime AND end_time = :endTime AND status NOT IN ('CANCELLED','COMPLETED') AND is_active = true LIMIT 1", nativeQuery = true)
