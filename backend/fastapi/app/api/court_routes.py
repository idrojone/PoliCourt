...
from app.schemas.court import CourtResponse, CourtFilteredResponse
...
@court_router.get("/courts/filtered", response_model=ApiResponse[CourtFilteredResponse])
def get_filtered_courts(db: Session = Depends(get_db)):
    return ApiResponse.success_response(CourtService(CourtRepository(db)).get_filtered_courts())