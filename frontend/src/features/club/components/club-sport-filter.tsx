...
interface ClubSportFilterProps {
    selected: string[];
    onToggle: (slug: string, checked: boolean) => void;
}

export const ClubSportFilter = ({ selected, onToggle }: ClubSportFilterProps) => {
    const [open, setOpen] = useState(false);
    const { data: sportsSlugs, isLoading } = useSportSlugsQuery();

    const sports = Array.isArray(sportsSlugs) ? sportsSlugs : [];
                ...
                <div className="p-2 space-y-2">
                    ...
                    {sports.map((sport) => (
                        <label key={sport.slug} className="flex items-center gap-2 cursor-pointer hover:bg-accent p-1 rounded">
                            ...
                            />
                            <span className="text-sm">{sport.name || sport.slug}</span>
                        </label>
                    ))}
                </div>
            </PopoverContent>
